...
//             "  %d %d              %d            %x          "
//             "%x    %d   %d %d %d %d %d\n",
//             i, elfInfo[i].i_sh_name, elfInfo[i].i_sh_type,
//             elfInfo[i].i_sh_addr, elfInfo[i].i_sh_offset,
//             elfInfo[i].i_sh_size, elfInfo[i].i_sh_entsize,
//             elfInfo[i].i_sh_flags, elfInfo[i].i_sh_link,
//             elfInfo[i].i_sh_info, elfInfo[i].i_sh_addralign);
//   }
//   fprintf(stderr, "\n\n");
// }
...
// static void sh_name(int index, Elf64_Info_Shdr *sHdrInfo, Elf64_Shdr
// *elfShdr) {
//   elfShdr->sh_name =
...
// static void sh_type(int index, Elf64_Info_Shdr *sHdrInfo, Elf64_Shdr
// *elfShdr) {
//   elfShdr->sh_type =
...
// static void sh_addr(int index, Elf64_Info_Shdr *sHdrInfo, Elf64_Shdr
// *elfShdr) {
//   elfShdr->sh_addr =
//       byte_get_little_endian(elfShdr->sh_addr, sizeof(elfShdr->sh_addr));
...
// static void sh_size(int index, Elf64_Info_Shdr *sHdrInfo, Elf64_Shdr
// *elfShdr) {
//   elfShdr->sh_size =
...
// static void sh_link(int index, Elf64_Info_Shdr *sHdrInfo, Elf64_Shdr
// *elfShdr) {
//   elfShdr->sh_link =
...
// static void sh_info(int index, Elf64_Info_Shdr *sHdrInfo, Elf64_Shdr
// *elfShdr) {
//   elfShdr->sh_info =
...
//                        Elf64_Shdr *elfShdr) {
//   elfShdr->sh_entsize =
//       byte_get_little_endian(elfShdr->sh_entsize,
//       sizeof(elfShdr->sh_entsize));
// }
...
int processSectionHeader(const char *inputFileName) {
  ...
  Elf64_Auxiliary_Shdr auxiliaryElf64Shdr;

  // shstrtab(sectionNumber - 1, sHdrStrTab, &sectionHdr[sectionNumber - 1]);

  for (int i = 0; i < sectionNumber; i++) {
    fread(&auxiliaryElf64Shdr, (i + 1) * sectionSize, sectionHeadersAddress,
          fileHandle);
